...
- Since India was under british colonial rule before 1947, a lot of people continue to associate few names as they were used earlier. For example: Victoria Station was renamed to Chhatrapi Shivaji Teminus. Sion is now named Shiv.
...
```
For example:
Shivaji nagar Depot => Shivaji Road Depot.
Mahalakshmi Marg=> Mahalakshmi Road


These are language specific problems that were encountered in the dataset. Marg is used to denote road and so on.
...
def update_name(name, mapping):
    ...
    return name
```
The following piece of code was used to correct the errors present. 
...
Zip codes were coarsely depicted with less characters. all zip codes in Mumbai begin with 4. So all data with zip codes as 98,76 were modified as 400098 and 400076.
```
<tag k="addr:postcode" v="61"/> => <tag k="addr:postcode" v="400061"/> 
<tag k="addr:postcode" v="98"/> => <tag k="addr:postcode" v="400098"/> 
<tag k="addr:postcode" v="71"/> => <tag k="addr:postcode" v="400071"/> 
```
...
- With the queries conducted, there was more of a focus on the study of node ???places??? rather than ways and their respective node 'waypoints'. A large number of nodes do not include any kind of addresses,Therefore we could compress the data by removing such orphaned waypoints.Furthermore, if ways were still needed, it would still be possible to remove any ???orphaned??? nodes that were only referenced in the removed relation and member tags.
- There are still several opportunities for cleaning and validation that I left unexplored. Of note, the data set is populated only from one source: OpenStreetMaps. While this crowdsourced repository pulls from multiple sources. It would have been an interesting exercise to validate and/or pull missing information (i.e. names) from the Google Maps API, since every node has latitude-longitude coordinates.
